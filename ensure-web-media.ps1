...
foreach ($source in $sources) {
  ...
  & $ffmpeg -hide_banner -y -i $source.FullName -map 0:v:0 -map 0:a? -vf "scale='min(1920,iw)':-2:flags=lanczos" -c:v libx264 -preset medium -crf 21 -pix_fmt yuv420p -movflags +faststart -c:a aac -b:a 192k $output 2>&1 | Add-Content -LiteralPath $logPath
