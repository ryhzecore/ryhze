...
while ($true) {
  ...
  if ($event) {
    ...
    if ($path -notmatch '[\\/]\.git([\\/]|$)' -and $path -notmatch '(mobile-conversion|web-media-conversion).*\.(log|lock)$' -and $path -notmatch '[\\/]library-data\.js$' -and $path -notmatch '[\\/]watch-sync\.(log|heartbeat)$') { $lastLocal = Get-Date }
  }
