...
async function passwordValid(password, stored) {
  const encoded =
    stored ||
    "scrypt$32768$8$3$00000000000000000000000000000000$" + "0".repeat(64);
  const parts = encoded.split("$");
  const actual = (await passwordHash(password, parts[4])).split("$")[5];
  return (
    timingSafeEqual(Buffer.from(actual, "hex"), Buffer.from(parts[5], "hex")) &&
    !!stored
  );
}
...
export async function session(request, env) {
  ...
  return env.DB.prepare(
    "SELECT u.id, u.username, u.role FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.token_hash=? AND s.expires_at>? AND u.disabled=0",
  )
    .bind(hash(value), now())
    .first();
}
async function startSession(env, user, remember) {
  const value = token(),
    seconds = remember ? 30 * 86400 : 12 * 3600;
  await env.DB.prepare(
    "INSERT INTO sessions (token_hash,user_id,expires_at) VALUES (?,?,?)",
  )
    .bind(hash(value), user.id, now() + seconds)
    .run();
  return json({ user: { username: user.username, role: user.role } }, 200, {
    "Set-Cookie": `${cookieName}=${value}; Path=/; HttpOnly; Secure; SameSite=Lax${remember ? "; Max-Age=" + seconds : ""}`,
  });
}
...
async function media(request, env, pathname) {
  ...
  try {
    ...
    return fail("Invalid media path");
  }
  if (
    !/^(Films|Games)\//.test(key) ||
...
async function handle(request, env) {
  ...
  if (url.hostname === "video.ryhze.com")
    return fail("Use the signed-in Ryhze media player.", 410);
  ...
  if (path === "/api/login" && request.method === "POST") {
    ...
    if (!username || username.length > 80 || password.length > 256)
      return fail("Incorrect user ID or password.", 401);
    const ipKey = hash(
      (request.headers.get("CF-Connecting-IP") || "local") +
        now().toString().slice(0, -5),
    );
    if (
      (await limited(env, "ip:" + ipKey, 30)) ||
      (await limited(env, "user:" + hash(username), 10))
    )
    ...
    const account = await env.DB.prepare(
      ...
      .bind(username)
      .first();
    if (!(await passwordValid(password, account?.password)) || account.disabled)
      return fail("Incorrect user ID or password.", 401);
  ...
  if (path === "/api/activate" && request.method === "POST") {
    ...
    if (
      ...
      return fail(
        "Use a valid invitation and a password of 12–256 characters.",
      );
    if (
      await limited(
        env,
        "activate:" + hash(request.headers.get("CF-Connecting-IP") || "local"),
        10,
      )
    )
      return fail("Too many attempts. Try again later.", 429);
    const invite = await env.DB.prepare(
      "SELECT i.user_id FROM invites i JOIN users u ON u.id=i.user_id WHERE i.token_hash=? AND i.expires_at>? AND i.used=0 AND u.disabled=0",
    )
      .bind(hash(body.token), now())
      .first();
    ...
    const result = await env.DB.batch([
      env.DB.prepare(
        "UPDATE users SET password=? WHERE id=? AND EXISTS (SELECT 1 FROM invites WHERE token_hash=? AND used=0 AND expires_at>?)",
      ).bind(encoded, invite.user_id, hash(body.token), now()),
      env.DB.prepare("UPDATE invites SET used=1 WHERE user_id=?").bind(
        invite.user_id,
      ),
    ...
    if (!result[0].meta.changes)
      return fail("This invitation is invalid or expired.", 400);
    return json({ ok: true });
  }
  if (path === "/api/logout" && request.method === "POST") {
    await env.DB.prepare("DELETE FROM sessions WHERE token_hash=?")
      .bind(hash(sessionToken(request)))
      .run();
  ...
  if (path.startsWith("/api/admin/")) {
    ...
    if (path === "/api/admin/users" && request.method === "GET")
      return json(
        (
          await env.DB.prepare(
            "SELECT id,username,role,disabled FROM users ORDER BY created_at",
          ).all()
        ).results,
      );
    if (path === "/api/admin/invite" && request.method === "POST") {
      ...
      if (!/^[\w.@+-]{2,80}$/.test(name))
        return fail("Use 2–80 letters, numbers, or email characters.");
      let account = await env.DB.prepare(
        "SELECT id,disabled FROM users WHERE username=? COLLATE NOCASE",
      )
        .bind(name)
        .first();
      if (!account) {
        ...
        await env.DB.prepare(
          ...
          .bind(account.id, name, now())
          .run();
      }
      if (account.disabled)
        return fail(
          "This account is disabled. Contact the administrator before issuing a new invitation.",
          409,
        );
      const value = token();
      await env.DB.batch([
        ...
        ).bind(hash(value), account.id, now() + 86400),
      ]);
    ...
    if (path === "/api/admin/disable" && request.method === "POST") {
      ...
      await env.DB.batch([
        env.DB.prepare("UPDATE users SET disabled=1 WHERE id=?").bind(
          String(body.id),
        ),
        env.DB.prepare("DELETE FROM sessions WHERE user_id=?").bind(
          String(body.id),
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      response = fail("Something went wrong. Please try again.", 503);
    }
    ...
    result.headers.set(
      ...
      "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data:; media-src 'self' blob:; connect-src 'self'; frame-src 'self'; frame-ancestors 'self'; base-uri 'none'; object-src 'none'; form-action 'self'",
    );
  ...
  async scheduled(event, env) {
    await env.DB.batch(
      ["sessions", "invites", "rate_limits"].map((table) =>
