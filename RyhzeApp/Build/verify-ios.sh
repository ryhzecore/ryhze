...
python3 ../Build/run-bounded.py 300 xcrun simctl bootstatus "$RYHZE_SIMULATOR_ID" -b
run_test() {
  local phase="$1"
  local target="$2"
  shift 2
  ...
  python3 ../Build/run-bounded.py 600 flutter build ios --simulator --debug --config-only --target="$target" "$@"
  local flutter_root
  flutter_root="$(sed -n 's/^FLUTTER_ROOT=//p' ios/Flutter/Generated.xcconfig | tr -d '\r')"
  python3 ../Build/run-bounded.py 900 xcodebuild test \
  ...
  python3 ../Build/check-ios-test-log.py "$phase" "build/ios-test-results/$phase.log"
}
