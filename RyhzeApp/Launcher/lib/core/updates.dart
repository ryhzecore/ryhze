...
class AppRelease {
  ...
  static Future<AppRelease?> verify(
    ...
    final extension = platform == 'windows'
        ...
        ? 'Linux.tar.gz'
        : 'Android.apk';
    final validPath = product == 'RACE'
        ? path == '/api/admin/race/releases/$version/RACE-$version-$extension'
        : path == '/releases/$version/Ryhze-$version-$extension' ||
              path ==
                  '/releases/$version/Ryhze-$version-build$build-$extension';
...
class AppUpdates extends ChangeNotifier {
  ...
  Future<void> download() async {
    ...
    try {
      ...
      await for (final chunk in response.stream.timeout(
        ...
        if (received == target.bytes ||
            DateTime.now().difference(lastReport).inMilliseconds >= 100) {
          _notify();
  ...
  static Future<String?> _install(File file, AppRelease release) async {
    if (Platform.isLinux) {
      final script = File('${file.parent.path}/update-linux.py');
      await script.writeAsString(
        await rootBundle.loadString('assets/update/update-linux.py'),
        flush: true,
      );
      final ready = File(
        '${file.parent.path}/handoff-$pid-${DateTime.now().microsecondsSinceEpoch}.txt',
      );
      final helper = await Process.start('/usr/bin/python3', [
        script.path,
        '--archive',
        file.path,
        '--sha256',
      ...
      try {
        for (var attempt = 0; attempt < 1200; attempt++) {
          if (await ready.exists()) {
            final response = await ready.readAsString();
            if (response == 'ready') {
              await ready.delete();
              exit(0);
            }
            if (response.startsWith('error:')) {
              throw FileSystemException(response);
            }
          }
          if (finished) {
            throw const FileSystemException(
              'Linux update helper exited before handoff',
            );
          }
          await Future<void>.delayed(const Duration(milliseconds: 100));
        }
        throw const FileSystemException('Linux update helper did not respond');
      } finally {
