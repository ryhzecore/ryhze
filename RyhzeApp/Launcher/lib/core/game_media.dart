...
// Reviewed focal points keep the subject inside Ryhze's existing landscape crop.
const steamArtworkFocalPoints = <String, double>{
  '1174180': 1.0,
  '2322010': .6,
  '1091500': .8,
};
...
String steamArtworkUrl(String id) => RegExp(r'^\d+$').hasMatch(id)
    ? 'https://shared.akamai.steamstatic.com/store_item_assets/steam/apps/$id/library_hero_2x.jpg'
    : '';
...
class GameMediaStore {
  ...
  GameMedia initial(LocalGame game) {
    try {
      final cached = jsonDecode(
        prefs.getString('game-media-v4:${game.id}:${game.storeId}') ?? '{}',
      );
      if (cached['media'] is Map) {
        return GameMedia.fromJson(Map<String, dynamic>.from(cached['media']));
      }
    } catch (_) {
      /* Fetch a clean copy. */
    }
    return GameMedia(
      artwork: game.source == 'Steam' ? steamArtworkUrl(game.storeId) : '',
    );
  }
  ...
  Future<GameMedia> _load(LocalGame game) async {
    ...
    final key = 'game-media-v4:${game.id}:${game.storeId}';
    GameMedia? cached;
    try {
      final saved =
