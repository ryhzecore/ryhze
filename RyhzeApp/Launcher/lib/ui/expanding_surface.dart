...
Future<T?> expandingSurface<T>({
  ...
  final route = RawDialogRoute<T>(
    ...
    transitionDuration: Duration(
      milliseconds: reduced ? 0 : surfaceTransitionMilliseconds,
    ),
    pageBuilder: (context, animation, secondary) => LayoutBuilder(
      builder: (context, constraints) {
        final media = MediaQuery.of(context);
        final available = constraints.biggest;
        final gutter = anchorToSource
            ? 8.0
            : available.width <= 700
            ? 20.0
        ...
        final target = anchorToSource && origin != null
            ? Rect.fromLTWH(
                origin.left.clamp(
                  gutter,
                  (available.width - gutter - w).clamp(gutter, double.infinity),
                ),
                origin.top.clamp(
                  top,
                  (available.height - media.viewInsets.bottom - 20 - h).clamp(
                    top,
                    double.infinity,
                  ),
                ),
