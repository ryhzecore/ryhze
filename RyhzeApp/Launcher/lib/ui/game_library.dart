  ...
  final allow = await showDialog<bool>(
    ...
    builder: (context) => AlertDialog(
      ...
      content: SizedBox(
        ...
        child: Text(
          library.isLinux
              ? 'Allow Ryhze to find your installed Steam games, including libraries on microSD cards? Steam handles Proton and game updates.\n\nWhile Ryhze is open, it checks game processes to show activity and remember when you last played. Game paths and play history stay on this device. Game IDs are sent to Steam for official artwork.\n\nYou can turn this off or clear your history in Installed games.'
              ...
              children: [
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    hintText: library.isLinux
                        ? '/home/deck/Games/MyGame/game'
                ...
                TextButton.icon(
                  onPressed: saving
                      ...
                      : () async {
                          try {
                            final picked = library.isLinux
                                ? (await openFile())?.path
                                : await GameLibrary.channel
                ...
                Text(
                  library.isLinux
                      ? 'Choose an executable Linux game. Find Windows games through your Steam library so Steam can launch them with Proton.'
    ...
    builder: (context, _) => PopupMenuButton<String>(
      ...
      itemBuilder: (_) => [
        if (library.permission != true)
          const RyhzeMenuItem(value: 'allow', child: Text('Set up my games')),
        if (library.permission == true) ...[
          const RyhzeMenuItem(value: 'add', child: Text('Add a game')),
          RyhzeMenuItem(
            value: 'scan',
...
class LocalGameCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => ListenableBuilder(
    ...
    builder: (context, _) => FutureBuilder<GameMedia>(
      ...
      initialData: media.initial(game),
      builder: (context, snapshot) {
        ...
        final title = RyhzeTitle(
          ...
          kind: 'game',
          label: game.source,
          status: library.status(game),
          description: '',
          categories: [lastPlayedText(game)],
        );
        final artwork = GameArtwork(
          snapshot.data?.artwork ?? media.initial(game).artwork,
          name: game.name,
        );
            ...
            await Navigator.of(context).push(
              PageRouteBuilder<void>(
                ...
                pageBuilder: (_, _, _) => LocalGameDetail(
                  ...
                  cover: snapshot.data?.artwork ?? media.initial(game).artwork,
                ),
...
class GameArtwork extends StatelessWidget {
  ...
  Widget build(BuildContext context) => url == 'asset:assets/art/valorant.png'
      ...
      : Image.network(
          ...
          fit: fit,
          alignment: Alignment(
            url.contains('library_hero')
                ? steamArtworkFocalPoints[RegExp(
                        r'/apps/(\d+)/',
          ...
          errorBuilder: (_, _, _) => url.contains('library_hero_2x.jpg')
              ? GameArtwork(
                  url.replaceFirst('library_hero_2x.jpg', 'library_hero.jpg'),
                  name: name,
                  fit: fit,
                  thumbnail: thumbnail,
                )
              : url.contains('library_hero')
              ? GameArtwork(
                  url.replaceFirst('library_hero.jpg', 'capsule_616x353.jpg'),
                  name: name,
                  fit: fit,
                  thumbnail: thumbnail,
                )
              : url.contains('capsule_616x353.jpg')
              ? Image.network(
                  url.replaceFirst('capsule_616x353.jpg', 'header.jpg'),
                  fit: fit,
                  filterQuality: FilterQuality.high,
  ...
  Widget build(BuildContext context) => ListenableBuilder(
    ...
    builder: (context, _) {
      ...
      return Wrap(
        ...
        runSpacing: 10,
        children: [
          if (library.missing.contains(game.id) ||
              library.launchErrors.containsKey(game.id))
            if (library.missing.contains(game.id) ||
                library.launchErrors.containsKey(game.id))
              Text(library.status(game)),
          if (library.missing.contains(game.id) ||
              library.launchErrors.containsKey(game.id)) ...[
            Pill(
              'Rescan',
            ...
            Pill(
              'Locate',
              onPressed: () => editLocalGame(context, library, game),
            ),
          ],
          Pill(
            playing
...
class _LocalGameDetailState extends State<LocalGameDetail>
  ...
  RyhzeTitle? get catalogueEntry => widget.state.titles
      .where(
        (t) =>
            t.isGame &&
            (gameNameKey(t.title) == gameNameKey(widget.game.name) ||
                (widget.game.source != 'Epic Games' &&
                    widget.game.storeId.isNotEmpty &&
                    t.storeId == widget.game.storeId)),
      )
      .firstOrNull;
  String get favouritesKey => 'local-organisation:${widget.state.scope}:game';
  bool get favourite => catalogueEntry != null
      ? widget.state.saved.contains(catalogueEntry!.id)
      : (widget.state.prefs.getStringList(favouritesKey) ?? []).contains(
          'favorite:${widget.game.id}',
        );
  Future<void> toggleFavourite() async {
    final title = catalogueEntry;
    if (title != null) {
      await widget.state.toggleSaved(title);
    } else {
      final items = (widget.state.prefs.getStringList(favouritesKey) ?? [])
          .toSet();
      final key = 'favorite:${widget.game.id}';
      items.contains(key) ? items.remove(key) : items.add(key);
      if (!await widget.state.prefs.setStringList(
        favouritesKey,
        items.toList(),
      )) {
        throw Exception('Your favourite could not be saved.');
      }
    }
    if (mounted) setState(() {});
  }
                                ...
                                children: [
                                  ...
                                  Pill(
                                    favourite
                                        ...
                                        : 'Add to list',
                                    iconOnly: true,
                                    icon: favourite ? Icons.check : Icons.add,
                                    height: 44,
                                    onPressed: () =>
                                        attempt(context, toggleFavourite),
                                  ),
                                ],
                              ),
                            ),
                                ...
                                child: Column(
                                  ...
                                  children:
                                      <Widget>[
                                            ...
                                            SizedBox(
                                              height: detailSectionGap(mobile),
                                            ),
                                            ArtworkHero(
                                              tag: widget.heroTag,
                                              image: '',
