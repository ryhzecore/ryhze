...
class _AdminPageState extends State<AdminPage> {
  ...
  Future<void> refresh() async {
    ...
    try {
      ...
      if (mounted && allowed) setState(() => error = e.toString());
    }
  }
  ...
  Future<void> invite() async {
    ...
    try {
      ...
      if (mounted && allowed) setState(() => link = result['url']);
      await refresh();
    } catch (e) {
      if (mounted && allowed) setState(() => error = e.toString());
    } finally {
      if (mounted && allowed) setState(() => busy = false);
    }
  }
  ...
  Future<void> disable(Map<String, dynamic> member) async {
    ...
    if (confirmed == true && mounted && allowed) {
  ...
  Future<void> setEngineAccess(Map<String, dynamic> member) async {
    if (!allowed || changingEngineMember != null) return;
    setState(() {
      changingEngineMember = member['id'];
      error = null;
    });
    try {
      await widget.state.api.request(
        ...
        body: {
          'id': member['id'],
          'allowed': member['engineDownloadAccess'] != true,
        },
      );
      ...
      if (mounted) setState(() => changingEngineMember = null);
    }
  }
        ...
        children: [
          ...
          for (final member in members) ...[
            ...
            if (member['disabled'] != 1 &&
                member['role'] != 'admin' &&
                (member['developerAccess'] == true ||
                    member['developerAccess'] == 1))
              Padding(
                ...
                child: Wrap(
                  ...
                  children: [
                    Text(
                      member['engineDownloadAccess'] == true
                          ? 'Engine downloads allowed'
                          : 'Engine downloads paused',
                    ...
                    Pill(
                      member['engineDownloadAccess'] == true
                          ? 'Pause engine access'
                          : 'Allow engine access',
                      onPressed: changingEngineMember != null
                          ? null
                          : () => setEngineAccess(
          ...
          const Text(
            'New developers need approval for engine downloads. Pausing downloads keeps their installed versions and projects.',
