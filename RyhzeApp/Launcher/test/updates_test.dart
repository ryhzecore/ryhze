...
void main() {
  ...
  test(
    'Linux uses the same signed update verification and rejects wrong package types',
    () async {
      final linux = {
        ...release,
        'platform': 'linux',
        'path': '/releases/1.0.4/Ryhze-1.0.4-Linux.tar.gz',
      };
      final verified = await AppRelease.verify(
        await signed(value: linux),
        'linux',
        publicKey: publicKey,
      );
      expect(verified!.filename, 'Ryhze-1.0.4-Linux.tar.gz');
      expect(AppUpdates(platform: 'linux').supported, true);
      await expectLater(
        AppRelease.verify(
          await signed(
            value: {
              ...linux,
              'path': '/releases/1.0.4/Ryhze-1.0.4-Android.apk',
            },
          ),
          'linux',
          publicKey: publicKey,
  ...
  test(
    'build-qualified packages must match signed build and platform',
    () async {
      for (final platform in ['windows', 'android', 'linux']) {
        ...
        final candidate = {
          ...release,
          'platform': platform,
          'path': '/releases/1.0.4/Ryhze-1.0.4-build5-$suffix',
        };
        expect(
          (await AppRelease.verify(
            await signed(value: candidate),
            platform,
            publicKey: publicKey,
          ))!.build,
          5,
        ...
        for (final path in [
          '/releases/1.0.4/Ryhze-1.0.4-build6-$suffix',
          '/releases/1.0.4/Ryhze-1.0.4-build5-$suffix?download=1',
          '/releases/1.0.4/../Ryhze-1.0.4-build5-$suffix',
        ]) {
          await expectLater(
            AppRelease.verify(
              await signed(value: {...candidate, 'path': path}),
              platform,
              publicKey: publicKey,
            ),
            throwsFormatException,
