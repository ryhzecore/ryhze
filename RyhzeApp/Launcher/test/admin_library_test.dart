  ...
  for (final width in [320.0, 390.0, 500.0, 700.0, 1000.0, 1280.0]) {
    testWidgets('Admin library header fits at $width', (tester) async {
      ...
      if (width > 480) {
        ...
        expect(menu.left, greaterThanOrEqualTo(tabs.right));
      }
      ...
      final raceCard = tester.getRect(
        find.byKey(const ValueKey('card-open-race-engine')),
      );
      final gameCard = tester.getRect(
        find.byKey(const ValueKey('card-open-internal-grand-theft-auto-vi')),
      );
      expect(raceCard.width, closeTo(gameCard.width, .1));
      expect(raceCard.height, closeTo(gameCard.height, .1));
      expect(raceCard.top, greaterThan(gameCard.bottom));
      ...
      expect(find.byType(RyhzeFooter), findsNothing);
      expect(tester.takeException(), isNull);
      await tester.runAsync(
