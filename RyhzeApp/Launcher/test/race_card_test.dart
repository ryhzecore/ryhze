...
void main() {
  ...
  for (final width in [390.0, 1280.0]) {
    testWidgets('RACE opens and closes with connected controls at $width', (
      tester,
    ) async {
      ...
      final state = await fixtureState(
        ...
        client: MockClient(
          (_) async => http.Response('{"schema":1,"releases":[]}', 200),
        ),
      );
      final key = GlobalKey();
      ...
      for (var frame = 1; frame <= 7; frame++) {
        ...
        await tester.runAsync(() => capture(key, 'race-open-${width.toInt()}-$frame'));
      }
      ...
      for (final opacity in tester.widgetList<Opacity>(
        ...
        expect(opacity.opacity, inInclusiveRange(.01, 1));
      }
      ...
      await tester.runAsync(() => capture(key, 'race-closing-${width.toInt()}'));
      for (var frame = 2; frame <= 8; frame++) {
        await tester.pump(const Duration(milliseconds: 33));
        await tester.runAsync(() => capture(key, 'race-close-${width.toInt()}-$frame'));
      }
      await tester.pumpAndSettle();
