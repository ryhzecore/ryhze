...
void main() {
  ...
  test('late expired requests and logout cannot clear a newer login', () async {
    final late = Completer<http.Response>();
    final api = RyhzeApi(store: MemorySession(), client: MockClient((r) async {
      if (r.url.path == '/api/login') return http.Response('{}', 200, headers: {'set-cookie': '__Host-ryhze_session=${'b' * 64}; Path=/'});
      return late.future;
    }));
    final pending = api.request('/api/library');
    final checked = expectLater(pending, throwsA(isA<ApiException>()));
    await api.request('/api/login', body: {});
    late.complete(http.Response('{"error":"Expired"}', 401));
    await checked;
    expect(api.authHeaders['Cookie'], contains('b' * 64));
    api.close();
  });
