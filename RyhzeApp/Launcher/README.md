...
Current source version: **0.1.0 (build 34)**, as defined in `pubspec.yaml`. See `../Docs/RELEASE_NOTES.md` for prior changes, `../Docs/GAME_LIBRARY.md` for the Windows game library, and `../Updates/README.md` for publishing future in-app updates.

The latest Website task changes are mapped in `../Docs/WEBSITE_PARITY.md`, including smooth corners, translucent frames, artwork flight, ambient sound defaults and the native brand introduction. Game artwork is losslessly transcoded from the website's AVIF payloads into real PNG files for native decoder compatibility.
...
```powershell
.\RyhzeApp\Build\flutter.ps1 pub get
.\RyhzeApp\Build\windows-plugins.ps1
.\RyhzeApp\Build\flutter.ps1 analyze
.\RyhzeApp\Build\flutter.ps1 test
.\RyhzeApp\Build\flutter.ps1 build windows --release
.\RyhzeApp\Build\package-windows.ps1
.\RyhzeApp\Build\flutter.ps1 build apk --release
.\RyhzeApp\Build\flutter.ps1 build appbundle --release
```
